...
par(mfrow=c(2,2))
with(PCData, {
  plot(PCData$Date_time, as.numeric(as.character(PCData$Global_active_power)), type="l",  xlab="", ylab="Global Active Power")
  plot(PCData$Date_time, as.numeric(as.character(PCData$Voltage)), type="l", xlab="datetime", ylab="Voltage")
  plot(PCData$Date_time,as.numeric(as.character(PCData$Sub_metering_1)),type="n",xlab="", ylab="Energy sub metering") 
  lines(PCData$Date_time,as.numeric(as.character(PCData$Sub_metering_2)), col = "red")
  lines(PCData$Date_time,as.numeric(as.character(PCData$Sub_metering_3)), col = "blue")
  legend("topright", lty = 1, col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), cex = 0.5)
  plot(PCData$Date_time, as.numeric(as.character(PCData$Global_reactive_power)), type="l",xlab="datetime",ylab="Global_reactive_power" )
})
dev.off()